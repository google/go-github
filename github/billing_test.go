...
func TestBillingService_GetAICreditUsage(t *testing.T) {
	...
	report, _, err := client.Billing.GetAICreditUsage(ctx, "o", opts)
	if err != nil {
	...
	if !cmp.Equal(report, want) {
		t.Errorf("Billing.GetAICreditUsage returned %+v, want %+v", report, want)
	}
...
func TestBillingService_GetAICreditUsage_FloatQuantities(t *testing.T) {
	t.Parallel()
	client, mux, _ := setup(t)
	...
	ctx := t.Context()
	report, _, err := client.Billing.GetAICreditUsage(ctx, "o", nil)
	if err != nil {
		t.Fatalf("Billing.GetAICreditUsage returned error: %v", err)
	}
