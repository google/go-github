...
func TestEnterpriseService_License(t *testing.T) {
	...
	mux.HandleFunc("/manage/v1/config/license", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, `{
			...
			"customTerms": true,
			"evaluation": false,
			"expireAt": `+referenceTimeStr+`,
			"insightsEnabled": true,
			"insightsExpireAt": `+referenceTimeStr+`,
			"learningLabEvaluationExpires": `+referenceTimeStr+`,
	...
	want := &LicenseStatus{
		...
		InsightsExpireAt:             &referenceTimestamp,
		LearningLabEvaluationExpires: &referenceTimestamp,
