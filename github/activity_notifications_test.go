...
func TestActivityService_ListNotification(t *testing.T) {
	...
	opt := &NotificationListOptions{
		All:           true,
		Participating: true,
		Since:         time.Date(2006, time.January, 0o2, 15, 0o4, 0o5, 0, time.UTC),
		Before:        time.Date(2007, time.March, 0o4, 15, 0o4, 0o5, 0, time.UTC),
	}
...
func TestActivityService_MarkNotificationsRead(t *testing.T) {
	...
	ctx := context.Background()
	_, err := client.Activity.MarkNotificationsRead(ctx, time.Date(2006, time.January, 0o2, 15, 0o4, 0o5, 0, time.UTC))
	if err != nil {
	...
	testNewRequestAndDoFailure(t, methodName, client, func() (*Response, error) {
		return client.Activity.MarkNotificationsRead(ctx, time.Date(2006, time.January, 0o2, 15, 0o4, 0o5, 0, time.UTC))
	})
}
...
func TestActivityService_MarkRepositoryNotificationsRead(t *testing.T) {
	...
	ctx := context.Background()
	_, err := client.Activity.MarkRepositoryNotificationsRead(ctx, "o", "r", time.Date(2006, time.January, 0o2, 15, 0o4, 0o5, 0, time.UTC))
	if err != nil {
	...
	testBadOptions(t, methodName, func() (err error) {
		_, err = client.Activity.MarkRepositoryNotificationsRead(ctx, "\n", "\n", time.Date(2006, time.January, 0o2, 15, 0o4, 0o5, 0, time.UTC))
		return err
	})

	testNewRequestAndDoFailure(t, methodName, client, func() (*Response, error) {
		return client.Activity.MarkRepositoryNotificationsRead(ctx, "o", "r", time.Date(2006, time.January, 0o2, 15, 0o4, 0o5, 0, time.UTC))
	})
}
