...
func TestPullRequestsService_CreateStack(t *testing.T) {
	t.Parallel()
	client, mux, _ := setup(t)
	input := PullRequestCreateStackRequest{PullRequests: []int{101, 102}}
...
func TestPullRequestsService_CreateStack_invalidOwner(t *testing.T) {
	...
	_, _, err := client.PullRequests.CreateStack(t.Context(), "%", "%", PullRequestCreateStackRequest{})
	testURLParseError(t, err)
}
